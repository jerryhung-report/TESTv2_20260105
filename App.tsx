...
// 吉祥物固定路徑 (口袋狗)
const POCKET_DOG_URL = 'https://raw.githubusercontent.com/tina-dev-pocket/assets/main/pocket-mascot.png';
...
const PERSONAS: Persona[] = [
  ...
  { title: '口袋土狗', desc: '台灣土狗型投資人擁有極強的環境適應力，不追求華麗報酬，而是能在各種市場條件下活得下來。分散、耐震、長期有效的基金配置，最符合他們的生存智慧。', riskLevel: 4, image: POCKET_DOG_URL },
...
const MOCK_FUNDS: Fund[] = [
  ...
  { code: '98638825', name: '統一大東協高股息基金(新台幣)', isin: 'TW00000006', currency: 'TWD', risk: 4, type: 'Core', desc: '深耕東協市場，鎖定當地高成長潛力之高息標的。', perf: '+16.8%', perf2y: '+32.4%', perf3y: '+48.5%' },
...
const ProgressBar = ({ current, total }: { current: number, total: number }) => (
  <div className="flex flex-col items-center gap-2 w-full max-w-md mx-auto mb-8">
    ...
    <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
      <div className="h-full bg-red-900 transition-all duration-700 ease-out" style={{ width: `${(current / total) * 100}%` }} />
    </div>
  </div>
);
...
const FundCard: React.FC<{ fund: Fund; isSelected: boolean; onToggle: (code: string) => void; externalLink?: string; isSelectable?: boolean; }> = ({ fund, isSelected, onToggle, externalLink, isSelectable = true }) => {
  ...
  return (
    <div className={`group relative bg-white border-2 rounded-[2rem] p-6 sm:p-8 transition-all duration-300 hover:shadow-xl cursor-pointer ${isSelected ? 'border-red-900 bg-red-50/5' : 'border-slate-100 hover:border-red-200 shadow-sm'}`} onClick={handleClick}>
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex-1 flex gap-5 items-start">
          <div className={`p-3 mt-1 rounded-full transition-all duration-300 ${isSelected ? 'bg-red-900 text-white' : 'bg-slate-50 text-slate-400 group-hover:bg-red-100 group-hover:text-red-900'}`}>
            {externalLink ? <ExternalLink size={24} className="group-hover:scale-110" /> : isSelected ? <CheckCircle size={24} /> : <Plus size={24} />}
          </div>
          <div className="space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs font-black px-2 py-1 bg-slate-900 text-white rounded-md tracking-wider">{fund.code}</span>
              <span className="text-xs font-bold px-2 py-1 border border-slate-200 text-slate-500 rounded-md bg-white">{fund.currency}</span>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">風險 RR{fund.risk}</span>
            </div>
            <h4 className="font-black text-xl text-slate-900 group-hover:text-red-900 transition-colors">{fund.name}</h4>
...
const Intro = ({ onStart }: { onStart: () => void }) => (
  <div className="relative min-h-[90vh] flex flex-col items-center justify-center px-4 bg-white py-[76px] overflow-hidden">
    <div className="absolute inset-0 pointer-events-none opacity-20">
      <Coins size={120} className="absolute top-[10%] left-[5%] animate-pulse" />
      <Rocket size={150} className="absolute bottom-[15%] right-[5%] rotate-12" />
    </div>
    <div className="relative z-10 max-w-5xl mx-auto text-center space-y-12 animate-fadeIn">
      <div className="inline-flex items-center gap-3 px-6 py-2 bg-red-50 rounded-full text-red-900">
        <BrainCircuit size={16} /> <span className="text-xs font-black tracking-widest uppercase">口袋投資風格分析</span>
      </div>
      <h1 className="font-black text-slate-900 leading-tight text-[36px] sm:text-[48px]">準備好探索<br/>你的基金人格了嗎？</h1>
      <p className="text-slate-400 font-medium text-[20px] max-w-2xl mx-auto">30題測驗，完成後即可立刻獲得您的<br/><span className="text-red-900/60 font-bold">投資風格定位分析報告</span> 與 <span className="text-slate-900 font-bold">基金推薦</span>！</p>
      <button onClick={onStart} className={`${BTN_DARK} px-16 py-6 rounded-[2rem] text-xl group overflow-hidden relative`}>
        <span className="relative z-10 flex items-center gap-3">啟動分析 <Sparkles size={24} className="group-hover:rotate-12 transition-transform" /></span>
      </button>
    </div>
  </div>
);
...
const InfoForm = ({ data, onChange, onNext }: { data: UserFormData; onChange: (d: UserFormData) => void; onNext: () => void }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto py-16 px-4 animate-fadeIn">
      <div className="bg-white border-2 border-slate-50 rounded-[3rem] p-12 shadow-2xl space-y-10">
        <div className="space-y-8">
          <div className="space-y-4">
            <label className="text-xs font-black text-slate-400 tracking-widest uppercase">性別</label>
            <div className="flex gap-4">
              {['男', '女'].map(opt => (
                <button key={opt} onClick={() => onChange({ ...data, gender: opt })} className={`flex-1 py-4 rounded-2xl font-black ${data.gender === opt ? 'bg-red-900 text-white' : 'bg-slate-50 text-slate-400'}`}>{opt}</button>
              ))}
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {['18-30歲', '31-40歲', '41-50歲', '51-60歲', '61歲以上'].map(opt => (
                <button key={opt} onClick={() => onChange({ ...data, age: opt })} className={`px-4 py-3.5 rounded-xl font-black ${data.age === opt ? 'bg-red-900 text-white' : 'bg-slate-50 text-slate-400'}`}>{opt}</button>
              ))}
            </div>
          </div>
          <div className="space-y-4 pt-6 border-t">
            <div className="relative">
              ...
              <input type="tel" className="w-full pl-14 pr-6 py-4.5 bg-slate-50 rounded-2xl font-black outline-none" value={data.phone} onChange={e => onChange({ ...data, phone: e.target.value })} placeholder="手機號碼" />
            </div>
            <div className="relative">
              <Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300" size={18} />
              <input type="email" className="w-full pl-14 pr-6 py-4.5 bg-slate-50 rounded-2xl font-black outline-none" value={data.email} onChange={e => onChange({ ...data, email: e.target.value })} placeholder="電子郵件" />
            </div>
          </div>
        </div>
        <button onClick={onNext} disabled={!isFormValid} className={`w-full ${isFormValid ? BTN_DARK : BTN_GHOST} py-5 rounded-2xl text-lg group`}>進入測驗階段 <ArrowRight size={22} className="group-hover:translate-x-1" /></button>
      </div>
    </div>
  );
};
...
const Quiz = ({ onComplete }: { onComplete: (ans: Record<number, number>) => void }) => {
  ...
  const handleSelect = (v: number) => {
    ...
    setAns(nextAns);
    if (idx === 18) setShowTransition(true);
    else if (idx < allQs.length - 1) setIdx(idx + 1);
    else onComplete(nextAns);
  };

  if (showTransition) return (
    <div className="max-w-4xl mx-auto py-24 px-4 animate-fadeIn">
      <div className="bg-slate-950 text-white rounded-[3rem] p-20 shadow-2xl text-center space-y-12">
        <Rocket size={80} className="mx-auto text-red-500 animate-bounce" />
        <h2 className="text-[36px] sm:text-[54px] font-black tracking-tighter leading-tight">太棒了！你已經完成一半</h2>
        <p className="text-[20px] text-slate-400 max-w-2xl mx-auto">現在要前往宇宙種樹囉~</p>
        <button onClick={() => { setShowTransition(false); setIdx(idx + 1); }} className={`${BTN_PRIMARY} px-16 py-6 rounded-[2rem] text-xl`}>開始種樹 <ArrowRight size={28} /></button>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-2xl mx-auto py-16 px-4 animate-fadeIn">
      ...
      <div className="bg-white border border-slate-100 rounded-[3rem] p-14 shadow-2xl text-center space-y-10 min-h-[400px] flex flex-col justify-center">
        ...
        <h3 className="text-xl sm:text-2xl font-black text-slate-900">{currQ.text || currQ.q}</h3>
        <div className="space-y-4">
          {currQ.type === 'range' && (
            <div className="grid grid-cols-7 gap-3">
              {[1, 2, 3, 4, 5, 6, 7].map(v => (
                <button key={v} onClick={() => handleSelect(v)} className={`aspect-square rounded-2xl font-black text-lg transition-all ${ans[currQ.id] === v ? 'bg-red-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}>{v}</button>
              ))}
              <div className="col-span-7 flex justify-between text-xs font-black text-slate-400 pt-2 tracking-widest px-1"><span>非常不認同</span><span>非常認同</span></div>
            </div>
          )}
          {currQ.type === 'bool' && (
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => handleSelect(7)} className="py-5 rounded-2xl font-black text-lg bg-red-900 text-white">是</button>
              <button onClick={() => handleSelect(1)} className="py-5 rounded-2xl font-black text-lg bg-slate-900 text-white">否</button>
            </div>
          )}
          {currQ.type === 'choice' && currQ.options?.map(o => (
            <button key={o.val} onClick={() => handleSelect(o.val)} className="w-full py-5 px-8 rounded-2xl font-bold text-lg text-left bg-slate-50 text-slate-500 flex justify-between items-center group hover:bg-red-900 hover:text-white transition-all">{o.text} <ArrowRight size={20} /></button>
          ))}
        </div>
      </div>
      <button onClick={() => idx > 0 && setIdx(idx - 1)} className="mt-8 mx-auto flex items-center gap-2 text-slate-400 font-bold hover:text-slate-900"><ArrowLeft size={18} /> 回上一題</button>
    </div>
  );
};
...
const Results = ({ persona, onContinue }: { persona: Persona; onContinue: () => void }) => {
  return (
    <div className="max-w-4xl mx-auto py-16 px-4 animate-fadeIn space-y-12">
      <div className="bg-slate-950 text-white rounded-[4rem] p-10 sm:p-20 relative overflow-hidden shadow-2xl border border-white/5">
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
          ...
          <div className="w-64 h-64 sm:w-96 sm:h-96 flex items-center justify-center relative">
             <img 
              src={POCKET_DOG_URL} 
              alt="口袋狗" 
      ...
      <button onClick={onContinue} className={`${BTN_PRIMARY} w-full py-6 rounded-[2rem] text-xl`}>查看推薦配置 <ArrowRight size={30} /></button>
  ...
  return (
    <div className="max-w-5xl mx-auto py-16 px-4 animate-fadeIn space-y-16">
      <div className="text-center space-y-6">
        ...
        <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">績效截止日：2024/12/31</p>
      </div>
      <div className="space-y-20 pb-40">
        <div className="space-y-8">
          <div className="flex items-center gap-4 px-2"><Globe size={26} className="text-red-900" /><h3 className="text-2xl font-black">核心配置</h3></div>
          <div className="grid gap-6">{coreFunds.map(f => (<FundCard key={f.code} fund={f} isSelected={selected.includes(f.code)} onToggle={onToggle} />))}</div>
        </div>
        <div className="space-y-8">
          <div className="flex items-center gap-4 px-2"><Satellite size={26} className="text-slate-900" /><h3 className="text-2xl font-black">衛星配置</h3></div>
          <div className="grid gap-6">{satelliteFunds.map(f => (<FundCard key={f.code} fund={f} isSelected={selected.includes(f.code)} onToggle={onToggle} externalLink={f.code === recs.etf ? "https://www.pocket.tw/" : undefined} isSelectable={f.code !== recs.etf} />))}</div>
        </div>
      </div>
      <div className="fixed bottom-8 left-0 right-0 z-50 px-4">
        <div className="max-w-4xl mx-auto bg-slate-950/95 backdrop-blur-xl text-white py-6 px-8 rounded-[3rem] shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-8 border border-white/10">
          <div className="flex items-center gap-8">
            <div className="relative p-4 bg-red-900/40 rounded-3xl"><ShoppingCart size={32} className="text-red-500" /><span className="absolute -top-1 -right-1 bg-red-600 w-7 h-7 rounded-full flex items-center justify-center text-xs font-black">{selected.length}</span></div>
            <div className="flex flex-col"><span className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">已選中標的</span><span className="text-2xl font-black">{selected.length} 檔基金</span></div>
          </div>
          <div className="flex gap-4 w-full sm:w-auto">
            <button onClick={onReset} className="px-8 py-5 rounded-2xl font-bold bg-white/5 hover:bg-white/10 text-slate-300 flex-1 sm:flex-none flex items-center justify-center gap-2">重新分析 <RefreshCcw size={16} /></button>
            <button onClick={() => window.open('https://my.cmoneyfund.com.tw/', '_blank')} className="px-14 py-5 rounded-2xl font-black bg-red-900 text-white hover:bg-red-800 flex-2 sm:flex-none flex items-center justify-center gap-2">立即申購 <ArrowRight size={18} /></button>
          </div>
        </div>
      </div>
    </div>
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <nav className="sticky top-0 z-[60] bg-white/80 backdrop-blur-xl border-b py-4 px-6 sm:px-12">
