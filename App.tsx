...
const PERSONAS: Persona[] = [
  ...
  { 
    title: '口袋吉娃', 
    desc: '吉娃娃型投資人情緒敏感、反應快速，容易受市場波動影響。透過分散配置與小額定期投入，有助於在高壓情緒中維持穩定度。', 
    riskLevel: 1,
  ...
  { 
    ...
    riskLevel: 2,
    image: 'https://raw.githubusercontent.com/tina-dev-pocket/assets/main/pocket-mascot.png' // 暫以此作為合成圖範例
  },
  { 
    title: '口袋馬爾', 
    desc: '馬爾濟斯型投資人資金規模不一定大，但對世界充滿好奇。他們偏好低門檻、可探索不同市場的配置，在控制風險下體驗視野擴張。', 
    riskLevel: 3,
  ...
  { 
    title: '口袋牧', 
    desc: '牧羊犬型投資人理性且高度系統化，相信規則勝過情緒。具備明確策略、可自動執行的投資方式，符合追求最佳化的思維。', 
    riskLevel: 3,
...
const Intro = ({ onStart }: { onStart: () => void }) => (
  <div className="max-w-4xl mx-auto text-center space-y-8 sm:space-y-10 py-10 sm:py-16 px-4 animate-fadeIn">
    ...
    <div className="space-y-6">
      <h1 className="text-[32px] font-black text-slate-900 leading-tight tracking-tighter">
        準備好探索<br/>
        你的基金人格了嗎？
      </h1>
      <p className="text-lg text-slate-500 max-w-2xl mx-auto leading-relaxed">
        30 題挑戰，完成後即可立刻獲得你的投資風格定位分析報告與基金推薦！
      </p>
    </div>
    <button onClick={onStart} className="px-16 py-[20px] bg-slate-900 text-white rounded-[2rem] font-black text-xl shadow-2xl hover:bg-red-800 hover:scale-105 transition-all duration-300">
            ...
            <div className="flex gap-4">
              {['男', '女'].map(opt => (
                <button 
                  ...
                  className={`flex-1 py-[12px] rounded-2xl font-black text-lg transition-all ${data.gender === opt ? 'bg-red-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}
                >
                  {opt}
                </button>
              ))}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {ageOptions.map(opt => (
                <button 
                  ...
                  className={`px-4 py-[10px] rounded-xl font-black text-sm transition-all ${data.age === opt ? 'bg-red-900 text-white shadow-md' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}
                >
                  {opt}
                </button>
              ))}
          ...
          <div className="space-y-6 pt-4 border-t border-slate-50">
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                <input 
                  type="tel" 
                  className="w-full pl-14 pr-6 py-4 bg-slate-50 rounded-2xl font-black outline-none focus:ring-2 focus:ring-red-900/10 transition-all" 
                  value={data.phone} 
  ...
  if (showTransition) {
    return (
      <div className="max-w-4xl mx-auto py-12 sm:py-20 px-4 animate-fadeIn">
        <div className="bg-slate-900 text-white rounded-[3rem] p-10 sm:p-16 shadow-2xl relative overflow-hidden text-center space-y-8">
          ...
          <div className="relative z-10 space-y-6">
            <div className="inline-flex p-4 bg-red-900/20 rounded-full text-red-500 mb-2">
          ...
          <button 
            ...
            className="relative z-10 px-12 py-5 bg-red-900 text-white rounded-[2rem] font-black text-xl hover:bg-red-800 transition-all hover:scale-105 flex items-center justify-center gap-3 mx-auto shadow-xl"
          >
            啟動決策核心 <ArrowRight size={24} />
          </button>
        </div>
      </div>
    );
        ...
        <div className="space-y-3">
          ...
          {currQ.type === 'bool' && (
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => handleSelect(7)} className="py-[14px] rounded-xl bg-slate-50 hover:bg-red-900 hover:text-white font-black text-lg transition-all">是</button>
              <button onClick={() => handleSelect(1)} className="py-[14px] rounded-xl bg-slate-50 hover:bg-slate-900 hover:text-white font-black text-lg transition-all">否</button>
            </div>
          )}
          {currQ.type === 'choice' && currQ.options?.map(o => (
            <button key={o.val} onClick={() => handleSelect(o.val)} className="w-full py-[10px] px-8 rounded-xl bg-slate-50 hover:bg-red-900 hover:text-white font-bold text-left transition-all flex justify-between items-center group">
...
const Results = ({ persona, onContinue }: { persona: Persona, onContinue: () => void }) => (
  <div className="max-w-4xl mx-auto py-8 sm:py-12 px-4 animate-fadeIn space-y-8 sm:space-y-10">
    <div className="bg-slate-950 text-white rounded-[3rem] sm:rounded-[4rem] p-8 sm:p-14 relative overflow-hidden shadow-2xl border border-slate-800">
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
        <div className="space-y-8 flex-1 text-center md:text-left">
          <div className="space-y-4">
            <span className="text-sm font-black text-red-500 tracking-[0.4em] uppercase border-b-2 border-red-900 pb-1">風格分析報告</span>
            <h2 className="text-5xl sm:text-6xl font-black tracking-tight">{persona.title}</h2>
          </div>
          <div className="space-y-4">
        ...
        <div className="relative group flex-shrink-0">
          <div className="absolute inset-0 bg-red-900/10 rounded-full blur-3xl group-hover:bg-red-900/20 transition-all duration-500"></div>
          <img 
            src={persona.image} 
            alt={persona.title} 
            className="relative z-10 w-56 h-56 sm:w-80 sm:h-80 object-contain animate-float drop-shadow-[0_20px_50px_rgba(153,27,27,0.3)]" 
            onError={e => (e.currentTarget.src = "https://api.iconify.design/noto:dog.svg")} 
          />
        </div>
      </div>
    </div>
    <button onClick={onContinue} className="w-full py-[24px] bg-red-900 text-white rounded-[2rem] font-black text-xl hover:bg-red-800 transition-all shadow-xl flex items-center justify-center gap-4 hover:scale-[1.02] active:scale-[0.98]">
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 sm:py-10 px-4 animate-fadeIn space-y-10 sm:space-y-12">
      <div className="text-center space-y-4">
        <h2 className="text-3xl sm:text-4xl font-black text-slate-900">您的專屬投資口袋</h2>
      ...
      <div className="sticky bottom-4 z-50 px-4">
        <div className="max-w-4xl mx-auto bg-slate-900 text-white p-3.5 sm:p-5 rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 border-b-4 border-red-900">
          <div className="flex items-center gap-4 sm:gap-6 w-full sm:w-auto px-2 sm:px-0">
            <div className="relative p-2 bg-red-900/20 rounded-xl">
              <ShoppingCart size={22} className="sm:size-7" />
              <span className="absolute -top-1 -right-1 bg-red-600 w-4.5 h-4.5 sm:w-5 sm:h-5 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-black">
                {selected.length}
              </span>
            </div>
            <div>
              <div className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider leading-none mb-0.5 sm:mb-1">已選中</div>
              <div className="text-base sm:text-lg font-black leading-none">{selected.length} 檔基金</div>
            </div>
          </div>
          <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
            <button 
              onClick={onReset} 
              className="flex-1 sm:flex-none px-4 sm:px-6 py-2 sm:py-2.5 bg-slate-800 rounded-xl sm:rounded-2xl font-bold flex items-center justify-center gap-1.5 hover:bg-slate-700 transition-all text-xs sm:text-sm whitespace-nowrap"
            >
              <RefreshCcw size={14} className="sm:size-4" /> 重新檢測
            </button>
            <button 
              onClick={() => window.open('https://my.cmoneyfund.com.tw/', '_blank')} 
              className="flex-2 sm:flex-none px-6 sm:px-10 py-2 sm:py-2.5 bg-red-900 rounded-xl sm:rounded-2xl font-black text-sm sm:text-lg hover:bg-red-800 transition-all flex items-center justify-center gap-1.5 sm:gap-2 whitespace-nowrap"
            >
              立即申購 <ArrowRight size={16} className="sm:size-[18px]" />
            </button>
          </div>
        </div>
      </div>
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-12 sm:pb-16">
      <nav className="sticky top-0 z-[60] bg-white/80 backdrop-blur-xl border-b border-slate-100 py-3 sm:py-4 px-6 sm:px-8">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 cursor-pointer group" onClick={() => setStep('intro')}>
            <div className="p-2 bg-slate-900 text-white rounded-xl group-hover:rotate-12 transition-transform"><Leaf size={18} /></div>
            <span className="text-base sm:text-lg font-black tracking-tighter italic uppercase">口袋<span className="text-red-900">投資風格分析</span></span>
          </div>
        </div>
      </nav>
      <main>
